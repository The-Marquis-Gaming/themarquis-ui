...
use super::SetUp::{
    ...
    get_optional_two, player_move, setup_free_game, setup_game_2_players, setup_game_new,
};

#[test]
fn should_create_new_free_game_session_for_two_players() {
    ...
    let some_players = get_optional_two();
...
fn should_panic_when_player_tries_to_join_session_twice() {
    // given a new game
    let eth_address = ETH_TOKEN_ADDRESS();
    let some_token = Option::Some(eth_address);
...
fn should_panic_when_game_is_initialized_with_unsupported_token() {
    ...
    let _ = marquis_game_dispatcher
        .create_session(
            Option::Some(token_address), Option::Some(amount), Option::None, required_players,
        );
}
