...
export async function createEntitySubscription(
  ...
  const wsClient = createClient({ url: process.env.NEXT_PUBLIC_TORII_WS! });
  const client = new GraphQLClient(process.env.NEXT_PUBLIC_TORII_URL!);
...
const isEntityUpdate = (componentNames: string[]) => {
  // create realm
  if (
    ["Realm", "Owner", "EntityMetadata", "Position"].every((element) =>
